...
echo "LOG_INDEX_DATE: ${LOG_INDEX_DATE}"
echo "LOG_FILTER_1 :  ${LOG_FILTER_1}"
echo "LOG_FILTER_2 : ${LOG_FILTER_2}"
echo "RAW_LOG_PATH : ${RAW_LOG_PATH}"
echo "PERSIST_LOGS : ${PERSIST_LOGS}"

echo "Running command: log-matcher-extender2.sh $LOG_INDEX_DATE $LOG_FILTER_1 $RAW_LOG_PATH $PERSIST_LOGS $LOG_FILTER_2"

mkdir $DATA_DIRECTORY/$LOG_FILTER_1 #This directory will hold the intermediate and processed log files for each
...
if [ "x$RAW_LOG_SOURCE"=="xremote" ];then
	...
	sshpass -p $SSHPASS ssh -o StrictHostKeyChecking=no $RAW_LOG_USER@$RAW_LOG_HOST cat /var/lib/mesos/slaves/40a92cc8-7dab-4a94-b092-f0ca85ae9579-S0/frameworks/703fd5ae-3915-48b9-a66b-9e54ccede103-0000/executors/$RAW_LOG_PATH/runs/latest/stdout > $DATA_DIRECTORY/$LOG_FILTER_1/raw_log && python3 log_processor2.py $DATA_DIRECTORY/$LOG_FILTER_1/raw_log "$LOG_FILTER_1" > $DATA_DIRECTORY/$LOG_FILTER_1/raw_log_2 && python3 log_processor2.py $DATA_DIRECTORY/$LOG_FILTER_1/raw_log_2 "$LOG_FILTER_2" > $DATA_DIRECTORY/$LOG_FILTER_1/${LOG_FILTER_1}_${LOG_FILTER_2}_source_processed.log
else
